...
const { copyRecursiveSync } = require('./helpers')
                                ...
                                if (line.includes('${')) {
                                    ...
                                    if (variableName !== 'theme.palette.mode') {
                                        ...
                                        getVariableValue(data, variableName).map(r => {
                                            const replaced = line.replace('src=', '').replace('{`', '').replace('`}', '').replace('${' + variableName + '}', r)
                                            ...
                                            const parentPathLight = replacedLight.substring(0, replacedLight.lastIndexOf('/')).replace(/\\/g, "/").trim()
                                            const parentPathDark = replacedDark.substring(0, replacedDark.lastIndexOf('/')).replace(/\\/g, "/").trim()
                                            
                                            checkAndCreate(`${pathConfig.starterKitTSXPath}/public${parentPathLight}`, () => {
                                                
                                                fs.copyFileSync(`${pathConfig.fullVersionTSXPath}/public${replacedLight}`, `${pathConfig.starterKitTSXPath}/public${replacedLight}`)
                                            })
                                            checkAndCreate(`${pathConfig.starterKitTSXPath}/public${parentPathDark}`, () => {
                                                
                                                fs.copyFileSync(`${pathConfig.fullVersionTSXPath}/public${replacedDark}`, `${pathConfig.starterKitTSXPath}/public${replacedDark}`)
                                            })
                                        ...
                                        const parentPathLight = imgPathLight.substring(0, imgPathLight.lastIndexOf('/')).replace(/\\/g, "/").trim()
                                        const parentPathDark = imgPathDark.substring(0, imgPathDark.lastIndexOf('/')).replace(/\\/g, "/").trim()

                                        checkAndCreate(`${pathConfig.starterKitTSXPath}/public${parentPathLight}`, () => {
                                            
                                            fs.copyFileSync(`${pathConfig.fullVersionTSXPath}/public${imgPathLight}`, `${pathConfig.starterKitTSXPath}/public${imgPathLight}`)
                                        })
                                        checkAndCreate(`${pathConfig.starterKitTSXPath}/public${parentPathDark}`, () => {
                                            
                                            fs.copyFileSync(`${pathConfig.fullVersionTSXPath}/public${imgPathDark}`, `${pathConfig.starterKitTSXPath}/public${imgPathDark}`)
                                        })
                                    ...
                                    const parentPath = fullPath.substring(0, fullPath.lastIndexOf('/')).replace(/\\/g, "/").trim()
                                        ...
                                        checkAndCreate(`${pathConfig.starterKitTSXPath}/public${parentPath}`, () => {
                                            ...
                                            fs.copyFileSync(`${pathConfig.fullVersionTSXPath}/public${fullPath}`, `${pathConfig.starterKitTSXPath}/public${fullPath}`)
                                        })
                                }
