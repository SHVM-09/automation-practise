...
interface ESLintConfig {
  ...
  settings?: {
    [key: string]: any;
    'import/parsers'?: { [key: string]: string[] };
    'import/resolver'?: {
      typescript?: { project: string };
    };
  };
  overrides?: { files: string[]; rules: { [key: string]: any } }[];
...
const removeTypescriptConfigs = (config: ESLintConfig): ESLintConfig => {
  ...
  config.rules = Object.fromEntries(
    Object.entries(config.rules ?? {}).filter(
      ([key]) => !key.startsWith('@typescript-eslint')
    )
  );
  ...
  if (config.settings) {
    ...
    if (config.settings['import/resolver'] && config.settings['import/resolver'].typescript) {
